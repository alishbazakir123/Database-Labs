...
(1, 1, 85000, 'Pending', '2026-05-16 08:44:46');
...
(1, 'Ali', 'ali@gmail.com', '123456', 1);

--
-- Triggers `users`
--
DELIMITER $$
CREATE TRIGGER `check_user_validation` BEFORE INSERT ON `users` FOR EACH ROW BEGIN

    DECLARE msg VARCHAR(255) DEFAULT '';

    IF NEW.email IS NULL OR NEW.email = '' THEN
        SET msg = CONCAT(msg, 'Email cannot be empty. ');
    END IF;

    IF NEW.password IS NULL OR NEW.password = '' THEN
        SET msg = CONCAT(msg, 'Password cannot be empty. ');
    ELSE
        IF CHAR_LENGTH(NEW.password) < 6 THEN
            SET msg = CONCAT(msg, 'Password must be at least 6 characters. ');
        END IF;
    END IF;
